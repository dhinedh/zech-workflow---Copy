...
async function createOrUpdateUser(email: string, password: string, name: string, role: any, companyId: string) {
    try {
...
async function main() {
    try {
        ...
        const adminCreds = await createOrUpdateUser(
            'makjuz@gmail.com',
            'Password@123',
            'Makjuz CEO',
            'SUPER_ADMIN',
        ...
        // 3. Create Freelancer
        const freelancerCreds = await createOrUpdateUser(
            'freelancer@makjuz.com',
            'Password@123',
            'Makjuz Freelancer',
            'FREELANCER', // This will work once schema is updated and client generated
            companyId
        );
